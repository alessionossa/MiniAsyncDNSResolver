...
//  SwiftNDI Sample
//
//  Created by Alessio Nossa on 05/10/2023.
//
...
extension Interface {
    fileprivate var kDNSServiceInterfaceIndex: UInt32 {
        switch self {
        case .default:
            return 0 //kDNSServiceInterfaceIndexAny
        case .localOnly:
        ...
        init<Handler: DNSSDQueryReplyHandler>(handler: Handler, _ continuation: AsyncThrowingStream<Handler.Record, Error>.Continuation) {
            self._handleRecord = { errorCode, data, length in
                guard errorCode == kDNSServiceErr_NoError else {
                    return continuation.finish(throwing: AsyncDNSResolver.Error.dnsService(code: errorCode))
                }
